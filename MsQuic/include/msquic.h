...
typedef enum QUIC_LOAD_BALANCING_MODE {
    ...
    QUIC_LOAD_BALANCING_SERVER_ID_FIXED,  // Encodes a fixed 4-byte value in
                                          // Server ID
...
typedef enum QUIC_CREDENTIAL_FLAGS {
    ...
    QUIC_CREDENTIAL_FLAG_CLIENT =
        0x00000001,  // Lack of client flag indicates server.
...
typedef enum QUIC_CONNECTION_SHUTDOWN_FLAGS {
    ...
    QUIC_CONNECTION_SHUTDOWN_FLAG_SILENT =
        0x0001,  // Don't send the close frame over the network.
...
typedef enum QUIC_SEND_RESUMPTION_FLAGS {
    ...
    QUIC_SEND_RESUMPTION_FLAG_FINAL =
        0x0001,  // Free TLS state after sending this ticket.
...
typedef enum QUIC_STREAM_SCHEDULING_SCHEME {
    QUIC_STREAM_SCHEDULING_SCHEME_FIFO =
        0x0000,  // Sends stream data first come, first served. (Default)
    QUIC_STREAM_SCHEDULING_SCHEME_ROUND_ROBIN =
        0x0001,  // Sends stream data evenly multiplexed.
    QUIC_STREAM_SCHEDULING_SCHEME_COUNT,  // The number of stream scheduling
                                          // schemes.
...
typedef enum QUIC_STREAM_OPEN_FLAGS {
    ...
    QUIC_STREAM_OPEN_FLAG_UNIDIRECTIONAL =
        0x0001,  // Indicates the stream is unidirectional.
    QUIC_STREAM_OPEN_FLAG_0_RTT =
        0x0002,  // The stream was opened via a 0-RTT packet.
    QUIC_STREAM_OPEN_FLAG_DELAY_ID_FC_UPDATES =
        0x0004,  // Indicates stream ID flow control limit updates for the
...
typedef enum QUIC_STREAM_START_FLAGS {
    ...
    QUIC_STREAM_START_FLAG_IMMEDIATE =
        0x0001,  // Immediately informs peer that stream is open.
    QUIC_STREAM_START_FLAG_FAIL_BLOCKED =
        0x0002,  // Only opens the stream if flow control allows.
    QUIC_STREAM_START_FLAG_SHUTDOWN_ON_FAIL =
        0x0004,  // Shutdown the stream immediately after start failure.
    QUIC_STREAM_START_FLAG_INDICATE_PEER_ACCEPT =
        0x0008,  // Indicate PEER_ACCEPTED event if not accepted at start.
    QUIC_STREAM_START_FLAG_PRIORITY_WORK =
        0x0010,  // Higher priority than other connection work.
...
typedef enum QUIC_STREAM_SHUTDOWN_FLAGS {
    ...
    QUIC_STREAM_SHUTDOWN_FLAG_GRACEFUL =
        0x0001,  // Cleanly closes the send path.
    QUIC_STREAM_SHUTDOWN_FLAG_ABORT_SEND =
        0x0002,  // Abruptly closes the send path.
    QUIC_STREAM_SHUTDOWN_FLAG_ABORT_RECEIVE =
        0x0004,  // Abruptly closes the receive path.
    QUIC_STREAM_SHUTDOWN_FLAG_ABORT =
        0x0006,  // Abruptly closes both send and receive paths.
    QUIC_STREAM_SHUTDOWN_FLAG_IMMEDIATE =
        0x0008,  // Immediately sends completion events to app.
    QUIC_STREAM_SHUTDOWN_FLAG_INLINE =
        0x0010,  // Process the shutdown immediately inline. Only for calls on
                 // callbacks. WARNING: Can cause reentrant callbacks!
...
typedef enum QUIC_SEND_FLAGS {
    ...
    QUIC_SEND_FLAG_ALLOW_0_RTT =
        0x0001,  // Allows the use of encrypting with 0-RTT key.
    QUIC_SEND_FLAG_START =
        0x0002,  // Asynchronously starts the stream with the sent data.
    QUIC_SEND_FLAG_FIN =
        0x0004,  // Indicates the request is the one last sent on the stream.
    QUIC_SEND_FLAG_DGRAM_PRIORITY =
        0x0008,  // Indicates the datagram is higher priority than others.
    QUIC_SEND_FLAG_DELAY_SEND = 0x0010,  // Indicates the send should be delayed
                                         // because more will be queued soon.
    QUIC_SEND_FLAG_CANCEL_ON_LOSS =
        0x0020,  // Indicates that a stream is to be cancelled when packet loss
                 // is detected.
    QUIC_SEND_FLAG_PRIORITY_WORK =
        0x0040,  // Higher priority than other connection work.
} QUIC_SEND_FLAGS;
...
typedef enum QUIC_DATAGRAM_SEND_STATE {
    ...
    QUIC_DATAGRAM_SEND_ACKNOWLEDGED_SPURIOUS,  // Acknowledged after being
                                               // suspected lost
...
typedef struct QUIC_EXECUTION_CONFIG {
    ...
    uint32_t PollingIdleTimeoutUs;  // Time before a polling thread, with no
                                    // work to do, sleeps.
    uint32_t ProcessorCount;
    _Field_size_(ProcessorCount) uint16_t
        ProcessorList[1];  // List of processors to use for threads.
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    _Function_class_(QUIC_CREDENTIAL_LOAD_COMPLETE) void(
        QUIC_API QUIC_CREDENTIAL_LOAD_COMPLETE)(_In_ HQUIC Configuration,
...
typedef struct QUIC_CERTIFICATE_PKCS12 {
    ...
    const char*
        PrivateKeyPassword;  // Optional: used if provided. Ignored if NULL
} QUIC_CERTIFICATE_PKCS12;

typedef void QUIC_CERTIFICATE;  // Platform specific certificate object
typedef void
    QUIC_CERTIFICATE_CHAIN;  // Platform specific certificate chain object
...
typedef struct QUIC_BUFFER {
    uint32_t Length;
    _Field_size_bytes_(Length) uint8_t* Buffer;
} QUIC_BUFFER;
...
typedef struct QUIC_NEW_CONNECTION_INFO {
    ...
    _Field_size_bytes_(CryptoBufferLength) const uint8_t* CryptoBuffer;
    _Field_size_bytes_(ClientAlpnListLength) const uint8_t* ClientAlpnList;
    _Field_size_bytes_(NegotiatedAlpnLength) const uint8_t* NegotiatedAlpn;
    _Field_size_bytes_opt_(ServerNameLength) const char* ServerName;
...
typedef enum QUIC_CIPHER_ALGORITHM {
    ...
    QUIC_CIPHER_ALGORITHM_CHACHA20 =
        0x6612,  // Not supported on Schannel/BCrypt
...
typedef enum QUIC_CIPHER_SUITE {
    ...
    QUIC_CIPHER_SUITE_TLS_CHACHA20_POLY1305_SHA256 =
        0x1303,  // Not supported on Schannel
} QUIC_CIPHER_SUITE;
...
typedef struct QUIC_STATISTICS {
    ...
    struct {
        ...
        uint64_t TotalPackets;  // QUIC packets; could be coalesced into fewer
                                // UDP datagrams.
        uint64_t RetransmittablePackets;
        uint64_t SuspectedLostPackets;
        uint64_t SpuriousLostPackets;  // Actual lost is (SuspectedLostPackets -
                                       // SpuriousLostPackets)
        ...
        uint32_t PersistentCongestionCount;  // Number of persistent congestion
                                             // events
    } Send;
    struct {
        uint64_t TotalPackets;  // QUIC packets; could be coalesced into fewer
                                // UDP datagrams.
        uint64_t ReorderedPackets;  // Packets where packet number is less than
                                    // highest seen.
...
typedef struct QUIC_STATISTICS_V2 {
    ...
    uint32_t EncryptionOffloaded : 1;  // At least one path successfully
    ...
    uint64_t
        TimingHandshakeFlightEnd;  // Processed all peer's Handshake packets
    ...
    uint64_t SendTotalPackets;  // QUIC packets; could be coalesced into fewer
                                // UDP datagrams.
    uint64_t SendRetransmittablePackets;
    uint64_t SendSuspectedLostPackets;
    uint64_t SendSpuriousLostPackets;  // Actual lost is (SuspectedLostPackets -
                                       // SpuriousLostPackets)
    ...
    uint32_t SendPersistentCongestionCount;  // Number of persistent congestion
                                             // events

    uint64_t RecvTotalPackets;  // QUIC packets; could be coalesced into fewer
                                // UDP datagrams.
    uint64_t RecvReorderedPackets;  // Packets where packet number is less than
                                    // highest seen.
    ...
    uint32_t
        DestCidUpdateCount;  // Number of times the destionation CID changed.

    uint32_t
        SendEcnCongestionCount;  // Number of congestion events caused by ECN.
...
#define QUIC_STATISTICS_V2_SIZE_1                   \
    QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, \
                                KeyUpdateCount)  // v2.0 final size
#define QUIC_STATISTICS_V2_SIZE_2                   \
    QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, \
                                DestCidUpdateCount)  // v2.1 final size
#define QUIC_STATISTICS_V2_SIZE_3                   \
    QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, \
                                SendEcnCongestionCount)  // v2.2 final size
...
typedef enum QUIC_PERFORMANCE_COUNTERS {
    ...
    QUIC_PERF_COUNTER_CONN_HANDSHAKE_FAIL,  // Total connections that failed
                                            // during handshake.
    QUIC_PERF_COUNTER_CONN_APP_REJECT,      // Total connections rejected by the
                                            // application.
    QUIC_PERF_COUNTER_CONN_RESUMED,         // Total connections resumed.
    QUIC_PERF_COUNTER_CONN_ACTIVE,          // Connections currently allocated.
    QUIC_PERF_COUNTER_CONN_CONNECTED,  // Connections currently in the connected
                                       // state.
    QUIC_PERF_COUNTER_CONN_PROTOCOL_ERRORS,  // Total connections shutdown with
                                             // a protocol error.
    QUIC_PERF_COUNTER_CONN_NO_ALPN,         // Total connection attempts with no
                                            // matching ALPN.
    ...
    QUIC_PERF_COUNTER_PKTS_DECRYPTION_FAIL,  // Total packets with decryption
                                             // failures.
    ...
    QUIC_PERF_COUNTER_CONN_QUEUE_DEPTH,       // Current connections queued for
                                              // processing.
    QUIC_PERF_COUNTER_CONN_OPER_QUEUE_DEPTH,  // Current connection operations
                                              // queued.
    QUIC_PERF_COUNTER_CONN_OPER_QUEUED,  // Total connection operations queued
                                         // ever.
    QUIC_PERF_COUNTER_CONN_OPER_COMPLETED,    // Total connection operations
                                              // processed ever.
    QUIC_PERF_COUNTER_WORK_OPER_QUEUE_DEPTH,  // Current worker operations
                                              // queued.
    QUIC_PERF_COUNTER_WORK_OPER_QUEUED,  // Total worker operations queued ever.
    QUIC_PERF_COUNTER_WORK_OPER_COMPLETED,  // Total worker operations processed
                                            // ever.
    QUIC_PERF_COUNTER_PATH_VALIDATED,  // Total path challenges that succeed
                                       // ever.
    QUIC_PERF_COUNTER_PATH_FAILURE,    // Total path challenges that fail ever.
    QUIC_PERF_COUNTER_SEND_STATELESS_RESET,  // Total stateless reset packets
                                             // sent ever.
    QUIC_PERF_COUNTER_SEND_STATELESS_RETRY,  // Total stateless retry packets
                                             // sent ever.
    QUIC_PERF_COUNTER_CONN_LOAD_REJECT,  // Total connections rejected due to
                                         // worker load.
...
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(QUIC_API* QUIC_SET_CONTEXT_FN)(
    _In_ _Pre_defensive_ HQUIC Handle, _In_opt_ void* Context);

typedef _IRQL_requires_max_(DISPATCH_LEVEL) void*(
    QUIC_API* QUIC_GET_CONTEXT_FN)(_In_ _Pre_defensive_ HQUIC Handle);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(
    QUIC_API* QUIC_SET_CALLBACK_HANDLER_FN)(_In_ _Pre_defensive_ HQUIC Handle,
                                            _In_ void* Handler,
                                            _In_opt_ void* Context);
...
#define QUIC_PARAM_HIGH_PRIORITY \
    0x40000000  // Combine with any param to make it high priority.

#define QUIC_PARAM_IS_GLOBAL(Param) \
    ((Param & 0x3F000000) == QUIC_PARAM_PREFIX_GLOBAL)

//
// Parameters for Global.
//
#define QUIC_PARAM_GLOBAL_RETRY_MEMORY_PERCENT 0x01000000  // uint16_t
#define QUIC_PARAM_GLOBAL_SUPPORTED_VERSIONS \
    0x01000001  // uint32_t[] - network byte order
#define QUIC_PARAM_GLOBAL_LOAD_BALACING_MODE \
    0x01000002  // uint16_t - QUIC_LOAD_BALANCING_MODE
#define QUIC_PARAM_GLOBAL_PERF_COUNTERS \
    0x01000003  // uint64_t[] - Array size is QUIC_PERF_COUNTER_MAX
...
#define QUIC_PARAM_GLOBAL_STATELESS_RESET_KEY \
    0x0100000B  // uint8_t[] - Array size is QUIC_STATELESS_RESET_KEY_LENGTH
//
...
#define QUIC_PARAM_CONFIGURATION_TICKET_KEYS \
    0x03000001  // QUIC_TICKET_KEY_CONFIG[]
#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
#define QUIC_PARAM_CONFIGURATION_VERSION_SETTINGS \
    0x03000002  // QUIC_VERSION_SETTINGS
#endif
...
#define QUIC_PARAM_CONFIGURATION_SCHANNEL_CREDENTIAL_ATTRIBUTE_W \
    0x03000003  // QUIC_SCHANNEL_CREDENTIAL_ATTRIBUTE_W
...
#define QUIC_PARAM_CONN_STREAM_SCHEDULING_SCHEME \
    0x0500000C  // QUIC_STREAM_SCHEDULING_SCHEME
#define QUIC_PARAM_CONN_DATAGRAM_RECEIVE_ENABLED \
    0x0500000D                                            // uint8_t (BOOLEAN)
...
#define QUIC_PARAM_CONN_TLS_SECRETS \
    0x05000013  // QUIC_TLS_SECRETS (SSLKEYLOGFILE compatible)
#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
...
#define QUIC_PARAM_TLS_SCHANNEL_CONTEXT_ATTRIBUTE_W \
    0x07000000  // QUIC_SCHANNEL_CONTEXT_ATTRIBUTE_W
typedef struct QUIC_SCHANNEL_CONTEXT_ATTRIBUTE_EX_W {
    unsigned long Attribute;
    unsigned long BufferLength;
    void* Buffer;
} QUIC_SCHANNEL_CONTEXT_ATTRIBUTE_EX_W;
#define QUIC_PARAM_TLS_SCHANNEL_CONTEXT_ATTRIBUTE_EX_W \
    0x07000001  // QUIC_SCHANNEL_CONTEXT_ATTRIBUTE_EX_W
...
#define QUIC_PARAM_STREAM_PRIORITY \
    0x08000003  // uint16_t - 0 (low) to 0xFFFF (high) - 0x7FFF (default)
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_SET_PARAM_FN)(
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_GET_PARAM_FN)(
        _When_(QUIC_PARAM_IS_GLOBAL(Param), _Reserved_)
            _When_(!QUIC_PARAM_IS_GLOBAL(Param), _In_ _Pre_defensive_)
                HQUIC Handle,
        _In_ uint32_t Param, _Inout_ _Pre_defensive_ uint32_t* BufferLength,
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_REGISTRATION_OPEN_FN)(
        _In_opt_ const QUIC_REGISTRATION_CONFIG* Config,
        _Outptr_ _At_(*Registration, __drv_allocatesMem(Mem))
            _Pre_defensive_ HQUIC* Registration);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(
    QUIC_API* QUIC_REGISTRATION_CLOSE_FN)(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Registration);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONFIGURATION_OPEN_FN)(
        _In_ _Pre_defensive_ HQUIC Registration,
        _In_reads_(AlpnBufferCount)
            _Pre_defensive_ const QUIC_BUFFER* const AlpnBuffers,
        _In_range_(>, 0) uint32_t AlpnBufferCount,
        _In_reads_bytes_opt_(SettingsSize) const QUIC_SETTINGS* Settings,
        _In_ uint32_t SettingsSize, _In_opt_ void* Context,
        _Outptr_ _At_(*Configuration, __drv_allocatesMem(Mem))
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONFIGURATION_LOAD_CREDENTIAL_FN)(
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    _Function_class_(QUIC_LISTENER_CALLBACK)
        QUIC_STATUS(QUIC_API QUIC_LISTENER_CALLBACK)(
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_LISTENER_OPEN_FN)(
        _In_ _Pre_defensive_ HQUIC Registration,
        _In_ _Pre_defensive_ QUIC_LISTENER_CALLBACK_HANDLER Handler,
        _In_opt_ void* Context,
        _Outptr_ _At_(*Listener, __drv_allocatesMem(Mem))
            _Pre_defensive_ HQUIC* Listener);

//
// Closes an existing listener.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(
    QUIC_API* QUIC_LISTENER_CLOSE_FN)(_In_ _Pre_defensive_ __drv_freesMem(Mem)
                                          HQUIC Listener);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_LISTENER_START_FN)(
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(
    QUIC_API* QUIC_LISTENER_STOP_FN)(_In_ _Pre_defensive_ HQUIC Listener);
...
typedef enum QUIC_CONNECTION_EVENT_TYPE {
    ...
    QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT =
        1,  // The transport started the shutdown process.
    QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_PEER =
        2,  // The peer application started the shutdown process.
    QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE =
        3,  // Ready for the handle to be closed.
    ...
    QUIC_CONNECTION_EVENT_RESUMED =
        13,  // Server-only; provides resumption data, if any.
    QUIC_CONNECTION_EVENT_RESUMPTION_TICKET_RECEIVED =
        14,  // Client-only; provides ticket to persist, if any.
    QUIC_CONNECTION_EVENT_PEER_CERTIFICATE_RECEIVED =
        15,  // Only with QUIC_CREDENTIAL_FLAG_INDICATE_CERTIFICATE_RECEIVED set
#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    QUIC_CONNECTION_EVENT_RELIABLE_RESET_NEGOTIATED =
        16,  // Only indicated if QUIC_SETTINGS.ReliableResetEnabled is TRUE.
    QUIC_CONNECTION_EVENT_ONE_WAY_DELAY_NEGOTIATED =
        17,  // Only indicated if QUIC_SETTINGS.OneWayDelayEnabled is TRUE.
    QUIC_CONNECTION_EVENT_NETWORK_STATISTICS =
        18,  // Only indicated if QUIC_SETTINGS.EnableNetStatsEvent is TRUE.
#endif
...
typedef struct QUIC_CONNECTION_EVENT {
    ...
    union {
        struct {
            ...
            _Field_range_(>, 0) uint8_t NegotiatedAlpnLength;
            _Field_size_(NegotiatedAlpnLength) const uint8_t* NegotiatedAlpn;
        } CONNECTED;
        struct {
            QUIC_STATUS Status;
        ...
        struct {
            _Field_range_(>, 0) uint32_t ResumptionTicketLength;
            _Field_size_(
        ...
        struct {
            BOOLEAN SendNegotiated;  // TRUE if sending one-way delay timestamps
                                     // is negotiated.
            BOOLEAN ReceiveNegotiated;  // TRUE if receiving one-way delay
                                        // timestamps is negotiated.
        } ONE_WAY_DELAY_NEGOTIATED;
        struct {
            uint32_t BytesInFlight;  // Bytes that were sent on the wire, but
                                     // not yet acked
            uint64_t PostedBytes;    // Total bytes queued, but not yet acked.
                                   // These may contain sent bytes that may have
                                   // portentially lost too.
            uint64_t IdealBytes;   // Ideal number of bytes required to be
                                   // available to  avoid limiting throughput
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    _Function_class_(QUIC_CONNECTION_CALLBACK)
        QUIC_STATUS(QUIC_API QUIC_CONNECTION_CALLBACK)(
...
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_OPEN_FN)(
        _In_ _Pre_defensive_ HQUIC Registration,
        _In_ _Pre_defensive_ QUIC_CONNECTION_CALLBACK_HANDLER Handler,
        _In_opt_ void* Context,
        _Outptr_ _At_(*Connection, __drv_allocatesMem(Mem))
            _Pre_defensive_ HQUIC* Connection);

//
// Closes an existing connection.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(
    QUIC_API* QUIC_CONNECTION_CLOSE_FN)(_In_ _Pre_defensive_ __drv_freesMem(Mem)
                                            HQUIC Connection);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_START_FN)(
        _In_ _Pre_defensive_ HQUIC Connection,
        _In_ _Pre_defensive_ HQUIC Configuration,
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_SET_CONFIGURATION_FN)(
        _In_ _Pre_defensive_ HQUIC Connection,
        _In_ _Pre_defensive_ HQUIC Configuration);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_SEND_RESUMPTION_FN)(
        _In_ _Pre_defensive_ HQUIC Connection,
        _In_ QUIC_SEND_RESUMPTION_FLAGS Flags, _In_ uint16_t DataLength,
        _In_reads_bytes_opt_(DataLength) const uint8_t* ResumptionData);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_COMP_RESUMPTION_FN)(
        _In_ _Pre_defensive_ HQUIC Connection, _In_ BOOLEAN Result);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_COMP_CERT_FN)(
        _In_ _Pre_defensive_ HQUIC Connection, _In_ BOOLEAN Result,
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    _Function_class_(QUIC_STREAM_CALLBACK)
        QUIC_STATUS(QUIC_API QUIC_STREAM_CALLBACK)(
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_OPEN_FN)(
        _In_ _Pre_defensive_ HQUIC Connection,
        _In_ QUIC_STREAM_OPEN_FLAGS Flags,
        _In_ _Pre_defensive_ QUIC_STREAM_CALLBACK_HANDLER Handler,
        _In_opt_ void* Context,
        _Outptr_ _At_(*Stream, __drv_allocatesMem(Mem))
            _Pre_defensive_ HQUIC* Stream);

//
// Closes a stream handle.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_STREAM_CLOSE_FN)(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Stream);

//
// Starts processing the stream.
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_START_FN)(
        _In_ _Pre_defensive_ HQUIC Stream, _In_ QUIC_STREAM_START_FLAGS Flags);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_SHUTDOWN_FN)(
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_SEND_FN)(
        _In_ _Pre_defensive_ HQUIC Stream,
...
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(
    ...
    _In_ _Pre_defensive_ HQUIC Stream, _In_ uint64_t BufferLength);

//
// Enables or disables stream receive callbacks.
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_RECEIVE_SET_ENABLED_FN)(
        _In_ _Pre_defensive_ HQUIC Stream, _In_ BOOLEAN IsEnabled);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_DATAGRAM_SEND_FN)(
        _In_ _Pre_defensive_ HQUIC Connection,
        _In_reads_(BufferCount)
            _Pre_defensive_ const QUIC_BUFFER* const Buffers,
        _In_ uint32_t BufferCount, _In_ QUIC_SEND_FLAGS Flags,
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_
    ...
    MsQuicOpenVersion(_In_ uint32_t Version,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) void QUIC_API
    MsQuicClose(_In_ _Pre_defensive_ const void* QuicApi);

#endif

_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_
    typedef QUIC_STATUS(QUIC_API* MsQuicOpenVersionFn)(
        _In_ uint32_t Version, _Out_ _Pre_defensive_ const void** QuicApi);

_IRQL_requires_max_(PASSIVE_LEVEL) typedef void(QUIC_API* MsQuicCloseFn)(
...
DECLSPEC_SELECTANY GUID MSQUIC_NPI_ID = {
    ...
    0xCD13,
    0x4CB1,
    {0x9C, 0xAE, 0xE0, 0x05, 0xC8, 0x55, 0x7A,
     0xBA}};  // C43138E3-CD13-4CB1-9CAE-E005C8557ABA

DECLSPEC_SELECTANY GUID MSQUIC_MODULE_ID = {
    0x698F7C72,
    0xC2E6,
    0x49CD,
    {0x8C, 0x39, 0x98, 0x85, 0x1D, 0x50, 0x19,
     0x01}};  // 698F7C72-C2E6-49CD-8C39-98851D501901
...
static NTSTATUS __MsQuicClientAttachProvider(
    _In_ HANDLE NmrBindingHandle, _In_ void* ClientContext,
...
#pragma warning(suppress : 6387)  // _Param_(2) could be '0' - by design.
        Status = NmrClientAttachProvider(
            NmrBindingHandle, Client, NULL, &ProviderContext,
            (const void**)&Client->ProviderDispatch);
        if (NT_SUCCESS(Status)) {
            KeSetEvent(&Client->RegistrationCompleteEvent, IO_NO_INCREMENT,
                       FALSE);
        } else {
...
static NTSTATUS __MsQuicClientDetachProvider(_In_ void* ClientBindingContext) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) __forceinline void MsQuicNmrClientDeregister(
...
_IRQL_requires_max_(PASSIVE_LEVEL) __forceinline NTSTATUS
    MsQuicNmrClientRegister(
        _Out_ HANDLE* ClientHandle, _In_ GUID* ClientModuleId,
        _In_ ULONG TimeoutMs  // zero = no wait, non-zero = some wait
    ) {
    NPI_REGISTRATION_INSTANCE* ClientRegistrationInstance;
    NTSTATUS Status = STATUS_SUCCESS;

    __MSQUIC_NMR_CLIENT* Client = (__MSQUIC_NMR_CLIENT*)ExAllocatePool2(
        POOL_FLAG_NON_PAGED, sizeof(*Client), 'cNQM');
    if (Client == NULL) {
        Status = STATUS_INSUFFICIENT_RESOURCES;
        goto Exit;
    }

    KeInitializeEvent(&Client->RegistrationCompleteEvent, SynchronizationEvent,
                      FALSE);
    ...
    Client->NpiClientCharacteristics.Length =
        sizeof(Client->NpiClientCharacteristics);
    Client->NpiClientCharacteristics.ClientAttachProvider =
        __MsQuicClientAttachProvider;
    Client->NpiClientCharacteristics.ClientDetachProvider =
        __MsQuicClientDetachProvider;

    ClientRegistrationInstance =
        &Client->NpiClientCharacteristics.ClientRegistrationInstance;
    ...
    Status = NmrRegisterClient(&Client->NpiClientCharacteristics, Client,
    ...
    Status = KeWaitForSingleObject(&Client->RegistrationCompleteEvent,
...
#endif
    QUIC_STATUS
    MsQuicOpen2(_Out_ _Pre_defensive_ const QUIC_API_TABLE** QuicApi) {
